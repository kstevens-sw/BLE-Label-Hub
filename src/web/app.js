...
function initInstantTooltips() {
  ...
  const syncTooltipPosition = (target) => {
    ...
    const top = Math.max(rect.top - 10, viewportPadding + tooltipRect.height);
  ...
  const showTooltip = (target) => {
    ...
    requestAnimationFrame(() => syncTooltipPosition(target));
  };
  ...
  document.addEventListener('focusout', () => {
    if (!document.activeElement || !document.activeElement.closest('[data-tooltip]')) {
      hideTooltip();
    }
  });

  window.addEventListener('scroll', () => {
    if (activeTarget) syncTooltipPosition(activeTarget);
  }, { passive: true });
...
function adjustLabelLength(delta) {
  ...
  if (newWidth !== currentWidth) {
    ...
    state.labelSize = normalizeLabelSizeForEditing({
      width: newWidth,
      height: tapeHeight,
      continuous: state.labelSize.continuous || false,
    });
    ...
    $('#mobile-custom-width').value = state.labelSize.width;
    $('#mobile-custom-height').value = state.labelSize.height;
  }
}

function normalizeLabelSizeForEditing(labelSize) {
  if (!labelSize) return { width: 40, height: 30, round: false };

  const normalized = { ...labelSize };
  if (normalized.round) return normalized;

  if (normalized.width < normalized.height) {
    [normalized.width, normalized.height] = [normalized.height, normalized.width];
  }

  return normalized;
}

function rotateElementsForWorkspace(elements, labelSize) {
  const oldWidth = labelSize.width;
  const oldHeight = labelSize.height;

  return elements.map((element) => {
    const centerX = element.x + (element.width / 2);
    const centerY = element.y + (element.height / 2);
    const rotatedCenterX = oldHeight - centerY;
    const rotatedCenterY = centerX;

    return {
      ...element,
      x: rotatedCenterX - (element.width / 2),
      y: rotatedCenterY - (element.height / 2),
      rotation: ((element.rotation || 0) + 90) % 360,
    };
  });
}

function normalizeDesignForEditing(elements, labelSize) {
  if (!labelSize || labelSize.round || labelSize.width >= labelSize.height) {
    return {
      elements,
      labelSize: normalizeLabelSizeForEditing(labelSize),
    };
  }

  return {
    elements: rotateElementsForWorkspace(elements, labelSize),
    labelSize: normalizeLabelSizeForEditing(labelSize),
  };
}

function applyCurrentLabelSize() {
...
function findMatchingLabelSizeKey(labelSize) {
  return Object.entries(LABEL_SIZES).find(([, preset]) => {
    if (!!preset.round !== !!labelSize.round) return false;
    if (!!preset.continuous !== !!labelSize.continuous) return false;
    if (preset.round) return preset.width === labelSize.width;

    return (
      (preset.width === labelSize.width && preset.height === labelSize.height) ||
      (preset.width === labelSize.height && preset.height === labelSize.width)
    );
  })?.[0] || null;
}

function rotateWorkspaceDesignSpace() {
  if (state.multiLabel.enabled) {
    setStatus('Rotate workspace is currently available for single-label layouts');
    return;
  }

  saveHistory();

  const oldWidth = state.labelSize.width;
  const oldHeight = state.labelSize.height;
  const newWidth = oldHeight;
  const newHeight = oldWidth;

  state.elements = rotateElementsForWorkspace(state.elements, state.labelSize);
  ...
  state.workspaceRotation = (state.workspaceRotation + 90) % 180;
  ...
  updatePropertiesPanel();
  detectTemplateFields();
  setStatus(`Workspace rotated to ${newWidth} x ${newHeight} mm`);
}

function syncLabelSizeUI() {
  const matchingKey = findMatchingLabelSizeKey(state.labelSize);
  const desktopSelect = $('#label-size');
  const mobileSelect = $('#mobile-label-size');

  if (matchingKey) {
    if (desktopSelect) desktopSelect.value = matchingKey;
    if (mobileSelect) mobileSelect.value = matchingKey;
    $('#custom-size')?.classList.add('hidden');
    $('#mobile-custom-size')?.classList.add('hidden');
  } else {
    if (desktopSelect) desktopSelect.value = 'custom';
    if (mobileSelect) mobileSelect.value = 'custom';
    $('#custom-size')?.classList.remove('hidden');
    $('#mobile-custom-size')?.classList.remove('hidden');
    if ($('#custom-width')) $('#custom-width').value = state.labelSize.width;
    if ($('#custom-height')) $('#custom-height').value = state.labelSize.height;
    if ($('#mobile-custom-width')) $('#mobile-custom-width').value = state.labelSize.width;
    if ($('#mobile-custom-height')) $('#mobile-custom-height').value = state.labelSize.height;
  }

  if ($('#custom-round')) $('#custom-round').checked = !!state.labelSize.round;
  if ($('#mobile-custom-round')) $('#mobile-custom-round').checked = !!state.labelSize.round;
  if ($('#custom-continuous')) $('#custom-continuous').checked = !!state.labelSize.continuous;
  if ($('#mobile-custom-continuous')) $('#mobile-custom-continuous').checked = !!state.labelSize.continuous;
}
...
function updateTemplateIndicator() {
  ...
  if (fieldCount) fieldCount.textContent = state.templateFields.length;
  ...
  if (hasFields) {
    ...
    if (fieldTags) fieldTags.innerHTML = '<span class="text-purple-400 italic">None</span>';
  ...
  // Update data count
  if (dataCount) dataCount.textContent = state.templateData.length;
  if (printCount) printCount.textContent = state.templateData.length;
...
function handleLabelSizeChange() {
  ...
  if (value === 'multi-label') {
    ...
    state.labelSize = normalizeLabelSizeForEditing({ width: w, height: h, round: false, continuous: false });
  } else {
    $('#custom-size').classList.add('hidden');
    const preset = LABEL_SIZES[value];
    if (preset) {
      state.labelSize = normalizeLabelSizeForEditing({ ...preset });
    }
  }
...
function handleCustomSizeChange() {
  ...
  state.labelSize = normalizeLabelSizeForEditing({ width: w, height: h, round: isRound, continuous: isContinuous });
  applyCurrentLabelSize();
  updatePrintSize();
  updateLengthAdjustButtons();
...
function applyMultiLabelConfig() {
  ...
  state.labelSize = normalizeLabelSizeForEditing({ width: labelWidth, height: labelHeight });
...
function exitMultiLabelMode() {
  ...
  $('#label-size').value = '40x30';
  state.labelSize = normalizeLabelSizeForEditing({ width: 40, height: 30, round: false });
  applyCurrentLabelSize();
...
function hideInfoDialog() {
  ...
  safeStorageSet('unified_ble_info_seen', 'true');
}
...
function shouldShowInfoOnLoad() {
  return !safeStorageGet('unified_ble_info_seen');
}
...
function handleImportFile(file) {
  ...
  reader.onload = (e) => {
    try {
      ...
      if (data.labelSize) {
        const normalizedDesign = normalizeDesignForEditing(state.elements, data.labelSize);
        state.elements = normalizedDesign.elements;
        state.labelSize = normalizedDesign.labelSize;
...
function updateElementsList() {
  ...
  if (state.elements.length === 0) {
    container.innerHTML = '<div class="px-3 py-2 text-gray-500 text-center">No elements</div>';
    return;
  }
  ...
  const html = state.elements.map((el, index) => {
    ...
    return `
      <button class="element-list-item w-full px-3 py-1.5 text-left text-gray-700 hover:bg-white/60 flex items-center gap-2 ${isSelected ? 'bg-white/70 text-gray-900' : ''}"
...
async function handleLoad(name) {
  ...
  const normalizedDesign = normalizeDesignForEditing(
    design.elements || [],
    design.labelSize || { width: 40, height: 30 }
  );
  state.elements = normalizedDesign.elements;
  state.labelSize = normalizedDesign.labelSize;
  state.selectedIds = [];
...
function checkCompatibility() {
  ...
  if (warnings.length > 0) {
    ...
    banner.id = 'compatibility-warning';
    banner.className = 'fixed top-0 left-0 right-0 z-[80] px-4 pt-4 pointer-events-none';
    banner.innerHTML = `
      <div class="mx-auto max-w-5xl bg-white/95 backdrop-blur border border-amber-200 text-gray-800 shadow-lg rounded-2xl pointer-events-auto">
        <div class="flex items-start gap-3 p-4 sm:p-5">
          ...
          <div class="flex-1 min-w-0">
            <div class="text-sm font-semibold text-gray-900">Printing is limited in this browser</div>
            <p class="text-sm text-gray-600 mt-0.5">
              ${warnings.join(' ')}
            </p>
          </div>
          <button id="dismiss-warning-btn" class="px-3 py-1.5 rounded-lg border border-gray-200 text-sm font-medium text-gray-700 hover:bg-gray-50">
            Dismiss
          </button>
        </div>
      </div>
...
function populatePrinterModelDropdown() {
  ...
  const makeSelect = $('#printer-make');
  const modelSelect = $('#printer-model');
  const currentModel = modelSelect?.value || state.printSettings.printerModel || 'auto';
  const currentDefinition = getPrinterDefinition(currentModel);
  const currentMake = currentDefinition?.group || 'auto';

  if (makeSelect) {
    makeSelect.innerHTML = '<option value="auto">Auto-detect</option>';
    const makes = [...new Set(defs.map(def => def.group || 'Other'))].sort();
  ...
  populateModelsForMake(currentMake, currentModel);

  const promptSelect = $('#prompt-model-select');
  if (promptSelect) {
    promptSelect.innerHTML = '<option value="">Select a model...</option>';
    for (const def of defs) {
      const option = document.createElement('option');
      option.value = def.id;
      option.textContent = `${def.group || 'Other'} - ${def.name}`;
      promptSelect.appendChild(option);
    }
  }
}

function populateModelsForMake(make, selectedModel = 'auto') {
  const select = $('#printer-model');
  if (!select) return;
  select.innerHTML = '';
  ...
  if (make === 'auto') {
    const option = document.createElement('option');
    option.value = 'auto';
    option.textContent = 'Auto-detect make and model';
    select.appendChild(option);
    return;
  }

  const definitions = getAllPrinterDefinitions()
    .filter(def => (def.group || 'Other') === make)
    .sort((left, right) => left.name.localeCompare(right.name));

  const defaultModel = make === 'NIIMBOT' && selectedModel === 'auto'
    ? 'niimbot-auto'
    : selectedModel;
  ...
  for (const def of definitions) {
    const option = document.createElement('option');
    option.value = def.id;
    const widthMm = def.widthBytes ? Math.round(def.widthBytes) : null;
    const dpi = def.dpi === 300 ? ', 300 DPI' : '';
    option.textContent = `${make} - ${def.name}${widthMm ? ` (${widthMm}mm${dpi})` : ''}${def.builtin ? '' : ' *'}`;
    select.appendChild(option);
  }
...
function init() {
  ...
  loadPrinterDefinitions().then(() => {
    ...
    const savedPrintSettings = safeStorageGet('unified_ble_print_settings');
    if (savedPrintSettings) {
      const settings = safeJsonParse(savedPrintSettings, null);
      if (settings && settings.printerModel) {
        const definition = getPrinterDefinition(settings.printerModel);
        const make = definition?.group || 'auto';
        $('#printer-make').value = make;
        populateModelsForMake(make, settings.printerModel);
      }
    }
  });
  ...
  state.labelSize = normalizeLabelSizeForEditing(state.labelSize);
  applyCurrentLabelSize();
  ...
  const printerMakeSelect = $('#printer-make');
  ...
  if (savedPrintSettings) {
    ...
    if (settings) {
      ...
      const definition = getPrinterDefinition(state.printSettings.printerModel);
      const make = definition?.group || 'auto';
