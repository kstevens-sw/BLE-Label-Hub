/**
 * Web Bluetooth transport for Unified BLE printers
 *
 * Features:
...
export class BLETransport {
  ...
  async connectGATT() {
    ...
    const writeCandidates = BLE.WRITE_CHAR_UUIDS || [BLE.WRITE_CHAR_UUID];
    const notifyCandidates = BLE.NOTIFY_CHAR_UUIDS || [BLE.NOTIFY_CHAR_UUID];

    this.writeChar = this.findCharacteristic(characteristics, writeCandidates)
      || characteristics.find(characteristic =>
        characteristic.properties.writeWithoutResponse || characteristic.properties.write
      );

    if (!this.writeChar) {
      throw new Error('Connected service has no writable Bluetooth characteristic');
    }
    ...
    try {
      this.notifyChar = this.findCharacteristic(characteristics, notifyCandidates)
  ...
  findCharacteristic(characteristics, candidateUuids) {
    const normalizedCandidates = new Set(candidateUuids.map(uuid => this.normalizeUuid(uuid)));
    return characteristics.find(characteristic =>
      normalizedCandidates.has(this.normalizeUuid(characteristic.uuid))
    ) || null;
  }
