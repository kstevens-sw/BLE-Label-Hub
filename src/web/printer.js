...
export async function loadPrinterDefinitions() {
  try {
    ...
    const niimbotLibrary = window.niimbluelib;
    if (niimbotLibrary?.modelsLibrary) {
      const niimbotDefinitions = niimbotLibrary.modelsLibrary.map(model => ({
        id: `niimbot-${model.model.toLowerCase().replaceAll('_', '-')}`,
        name: `NIIMBOT ${model.model.replaceAll('_', ' ')}`,
        group: 'NIIMBOT',
        description: `${model.printheadPixels}px BLE label printer`,
        protocol: 'niimbot',
        widthBytes: Math.ceil(model.printheadPixels / 8),
        dpi: model.dpi || 203,
        alignment: 'center',
        rotated: false,
        tape: false,
        ...
        namePatterns: [model.model.replaceAll('_', ' '), model.model.replaceAll('_', '-')],
        labelPresets: model.printheadPixels <= 192 ? 'd-series' : 'm-series',
        niimbotTask: niimbotLibrary.findPrintTask?.(model.model) || null,
        builtin: true,
      }));
      _builtinDefinitions.push(...niimbotDefinitions);
    }
  } catch (e) {
...
export function getAvailableProtocols() {
  return [
    ...
    { value: 'niimbot', label: 'NIIMBOT BLE (Auto-detect)' },
  ];
}
...
export function getPrinterProtocol(deviceName, modelOverride = 'auto') {
  return _resolveConfig(deviceName, modelOverride).protocol;
}
...
export async function print(transport, rasterData, options = {}) {
  ...
  if (isNiimbot) {
    await transport.printRaster(rasterData, {
      density,
      niimbotTask: _resolveConfig(deviceName, printerModel).definition?.niimbotTask,
    });
  } else if (isTSPL) {
