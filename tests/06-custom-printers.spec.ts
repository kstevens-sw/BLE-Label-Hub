...
test.describe.serial('Custom Printer Definitions', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.evaluate(() => localStorage.removeItem('unified_ble_custom_printers'));
  });
  ...
  test('includes a built-in Jadens JD-468BT definition', async ({ page }) => {
    // Given the application has loaded its built-in printer definitions
    // When the printer definitions manager is opened
    await page.click('#print-settings-btn');
    await page.click('#manage-printers-btn');

    // Then the JD-468BT is available with the expected TSPL configuration
    const row = page.locator('#printer-defs-list > div').filter({ hasText: 'JD-468BT' });
    await expect(row).toBeVisible();

    await row.locator('.pdef-edit-btn').click();
    await expect(page.locator('#pdef-protocol')).toHaveValue('tspl');
    await expect(page.locator('#pdef-width')).toHaveValue('102');
    await expect(page.locator('#pdef-dpi')).toHaveValue('203');
  });

  test('includes NIIMBOT BLE models from the protocol library', async ({ page }) => {
    // Given the NIIMBOT model library is loaded
    // When the printer definitions manager is opened
    await page.click('#print-settings-btn');
    await page.click('#manage-printers-btn');

    // Then common narrow, desktop, and wide BLE models are selectable
    await expect(page.locator('#printer-defs-list')).toContainText('NIIMBOT D11');
    await expect(page.locator('#printer-defs-list')).toContainText('NIIMBOT B21');
    await expect(page.locator('#printer-defs-list')).toContainText('NIIMBOT B3S');
  });
